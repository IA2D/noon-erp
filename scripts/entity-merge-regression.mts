...
const merged=mergeDuplicateEntity('CUSTOMER','c2','c1','admin','سجل مكرر',{customers,vendors:[],employees:[],contracts:[]}); assert.equal(merged.ok,true); assert.equal(merged.customers.find(x=>x.id==='c2')?.isActive,false); assert.equal(merged.customers.find(x=>x.id==='c2')?.mergedIntoId,'c1'); assert.equal(merged.customers.find(x=>x.id==='c1')?.openingBalance,30);
const blocked=mergeDuplicateEntity('CUSTOMER','c2','c1','admin','مرة أخرى',{customers:[{...customers[0],linkedAccountId:'a1'},{...customers[1],linkedAccountId:'a2'}],vendors:[],employees:[],contracts:[]}); assert.equal(blocked.ok,false);

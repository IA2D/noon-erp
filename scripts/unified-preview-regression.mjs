...
assert.match(custody, /const printLines = \[/);
assert.match(custody, /تصفية \$\{settlement\.settlementNumber\}/);
assert.match(custody, /البنود المحاسبية لصرف وتصفية العهدة/);
...
console.log('UNIFIED_PREVIEW_REGRESSION_OK listedReports=true directModulesUsePreview=true noDirectWindowPrint=true masterScheme=cash-movement portrait=true ownerRestoredAfterPreview=true');

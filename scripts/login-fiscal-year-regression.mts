...
assert.match(app, /function currentReportingYear\(\): string/);
assert.match(app, /function reportingYearOptions\(currentYear = new Date\(\)\.getFullYear\(\)\): string\[\]/);
assert.match(app, /year <= effectiveCurrentYear \+ 1/);
assert.match(app, /function restoredReportingYear\(\): string/);
assert.match(app, /useState<string>\(restoredReportingYear\)/);
assert.match(app, /sessionStorage\.getItem\(REPORTING_YEAR_SESSION_KEY\)/);
assert.match(app, /useMemo\(reportingYearOptions, \[\]\)/);
assert.match(app, /setReportingYear\(currentReportingYear\(\)\)/);
assert.doesNotMatch(app, /current - 5/);
assert.doesNotMatch(app, /current \+ 2/);
assert.doesNotMatch(app, /isAllowedReportingYear\(remembered\)/);
...
assert.match(reports, /const start = `\$\{year\}-01-01`/);
assert.match(reports, /const end = `\$\{year\}-12-31`/);

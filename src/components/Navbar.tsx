...
export default function Navbar({ user, onLogout, onNavigate, notificationCount, theme, toggleTheme, searchData, allowedModules, showRefreshButton = true, fiscalYear }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 ms-auto">
        {fiscalYear && (
          <div
            data-testid="navbar-fiscal-year"
            className="hidden md:flex items-center rounded-xl border border-white/40 bg-white/10 px-3 py-2.5 text-xs font-bold text-white whitespace-nowrap dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
            title={`السنة المالية النشطة: ${fiscalYear}`}
          >
            السنة المالية: <bdi dir="ltr" className="ms-1">{fiscalYear}</bdi>
          </div>
        )}
        <button

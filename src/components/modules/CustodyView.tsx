    ...
    return (
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button type="button" data-enter-nav="add-line" onClick={addItem} className="text-xs font-bold px-2.5 py-1.5 rounded-lg bg-sky-50 hover:bg-sky-100 text-sky-700 dark:bg-sky-500/20 dark:hover:bg-sky-500/30 dark:text-sky-300 cursor-pointer flex items-center gap-1">
          ...
          <ModalShell id="custody-settle" open={!!settleTarget} title={`تصفية ${settleTarget.custodyNumber} بالمستندات`} icon={FileSignature} onClose={() => { setSettleTarget(null); setSettleMaximized(false); }} footer={null} closeOnBackdrop={false} size="full" maxWidth="max-w-[96vw]" minWidth={900} minHeight={560} maximized={settleMaximized} onToggleMaximize={() => setSettleMaximized(value => !value)} bodyClassName="p-0">
            <form onSubmit={handleSettle} className="flex h-full min-h-0 flex-col" data-enter-scope="">
                ...
                {settleTarget.settlements.length > 0 && (
                  <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-3">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button type="button" onClick={() => { setSettlementEditId(null); setSettleItems([]); setSettlementAttachments([]); setApAccountId(''); }} className="px-3 py-1 rounded-lg bg-sky-50 text-sky-700 dark:bg-sky-500/20 dark:text-sky-300 dark:hover:bg-sky-500/30 text-xs font-bold">إضافة تصفية جديدة</button>
                    </div>
              ...
              <div className="shrink-0 p-4 border-t border-slate-200 dark:border-slate-700 flex justify-end gap-3 bg-white dark:bg-slate-900">
                ...
                <button type="submit" className="px-5 py-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:hover:bg-emerald-500/30 dark:text-emerald-300 rounded-xl text-sm font-bold shadow-lg cursor-pointer">تصفية وترحيل القيد</button>
              </div>
            </form>
          </ModalShell>
        );

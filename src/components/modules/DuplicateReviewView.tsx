...
interface Props { customers: Customer[]; vendors: Vendor[]; employees: Employee[]; currentUserName: string; onMerge: (result: { customers: Customer[]; vendors: Vendor[]; employees: Employee[] }, details: string) => void }
const labels: Record<MergeEntityKind,string> = { CUSTOMER:'العملاء', VENDOR:'الموردون', EMPLOYEE:'الموظفون' };

export default function DuplicateReviewView({ customers, vendors, employees, currentUserName, onMerge }: Props) {
